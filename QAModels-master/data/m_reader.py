...
class MnemonicReader(nn.Module):
    ...
    def __init__(self, args, normalize=True):
        ...
        self.char_rnn = layers.StackedBRNN(
            input_size=args.char_embedding_dim,#50
            hidden_size=args.char_hidden_size, #RNN units 的数目为64
            num_layers=1, #1层
            dropout_rate=args.dropout_rnn, #0.3 的丢失率
            dropout_output=args.dropout_rnn_output,#true,是否丢弃RNN输出
            concat_layers=False,
            rnn_type=self.RNN_TYPES[args.rnn_type],#使用lstm
            padding=args.rnn_padding,#false,明确考虑RNN编码中的填充，序列不定长不填充
        )
         #300+64*2+
        ...
        self.encoding_rnn = layers.StackedBRNN(
            ...
            hidden_size=args.hidden_size,#128个RNN units，– 隐层的特征数量
            num_layers=1,
        ...
        for i in range(args.hop): #2个hop
            # interactive aligner
            self.interactive_aligners.append(layers.SeqAttnMatch(doc_hidden_size, identity=True)) #输入为256，query与context，这一层求出query在context上的attention
